...
export const MPMC_RING_CACHE_LINE_BYTES = 64;

const RING_MAGIC = 0x4d50_4d43;
const RING_ABI_VERSION = 1;
const HEADER_WORDS = MPMC_RING_CACHE_LINE_BYTES / Int32Array.BYTES_PER_ELEMENT;
const ENQUEUE_BYTE_OFFSET = MPMC_RING_CACHE_LINE_BYTES;
const DEQUEUE_BYTE_OFFSET = MPMC_RING_CACHE_LINE_BYTES * 2;
const SLOTS_BYTE_OFFSET = MPMC_RING_CACHE_LINE_BYTES * 3;
const SLOT_WORDS = 2;
...
/**
 * A bounded multi-producer/multi-consumer queue of unsigned 32-bit handles.
 *
 * Every slot owns a sequence number. Producers publish payloads by advancing the sequence, and
 * consumers recycle slots by advancing it by one capacity. The queue itself is a non-owning view;
 * the surrounding SharedBuffer lease remains responsible for lifetime.
 */
export class MpmcRingBufferU32 {
  ...
  static byteLengthFor(capacity: number): number {
    ...
    return alignTo(SLOTS_BYTE_OFFSET + capacity * SLOT_BYTES, MPMC_RING_CACHE_LINE_BYTES);
  }

  /** Initializes the ring before its SharedBuffer is published to Workers. */
  static initialize(
    buffer: SharedRingBufferSource,
    byteOffset: number,
    capacity: number,
    ...
    const byteLength = MpmcRingBufferU32.byteLengthFor(capacity);
    ...
    return new MpmcRingBufferU32(buffer, byteOffset, capacity, byteLength);
  }

  static attach(buffer: SharedRingBufferSource, byteOffset: number): MpmcRingBufferU32 {
    validateByteOffset(byteOffset);
    const header = buffer.int32Array(byteOffset, HEADER_WORDS);
    if (Atomics.load(header, MAGIC_INDEX) !== RING_MAGIC) {
      throw new RangeError("shared memory does not contain an initialized MpmcRingBufferU32");
    }
    const version = Atomics.load(header, VERSION_INDEX);
    if (version !== RING_ABI_VERSION) {
      throw new RangeError(`unsupported MpmcRingBufferU32 ABI version: ${version}`);
    }
    const capacity = Atomics.load(header, CAPACITY_INDEX) >>> 0;
    validateCapacity(capacity);
    const byteLength = Atomics.load(header, BYTE_LENGTH_INDEX) >>> 0;
    if (
      byteLength !== MpmcRingBufferU32.byteLengthFor(capacity) ||
      Atomics.load(header, SLOTS_OFFSET_INDEX) !== SLOTS_BYTE_OFFSET
    ) {
      throw new RangeError("invalid MpmcRingBufferU32 layout");
    }
  ...
  tryPush(value: number): boolean {
    ...
    while (true) {
      ...
      this.#payloads[sequenceIndex + 1] = value;
  ...
  /** Blocks the current Worker while the queue is full. */
  push(value: number): void {
    ...
    while (!this.tryPush(value)) {
      ...
      waitForAtomicChangeBlocking(this.#slots, sequenceIndex, sequence, "MpmcRingBufferU32.push");
    }
  }
  ...
  tryPop(): number | undefined {
    ...
    while (true) {
      ...
      const value = this.#payloads[sequenceIndex + 1]!;
  ...
  /** Blocks the current Worker while the queue is empty. */
  pop(): number {
    while (true) {
      ...
      waitForAtomicChangeBlocking(this.#slots, sequenceIndex, sequence, "MpmcRingBufferU32.pop");
    }
  }
...
function validateByteOffset(byteOffset: number): void {
  ...
  if (byteOffset % MPMC_RING_CACHE_LINE_BYTES !== 0) {
    throw new RangeError(`byteOffset must be ${MPMC_RING_CACHE_LINE_BYTES}-byte aligned`);
  }
}
