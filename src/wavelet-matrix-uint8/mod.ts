...
/** An immutable eight-level binary wavelet matrix over the complete byte domain. */
export class WaveletMatrixUint8 {
  readonly length: number;
  ...
  static from(values: ArrayLike<number>): WaveletMatrixUint8 {
    return new WaveletMatrixUint8({ values });
  }

  static fromSnapshot(snapshot: Uint8Array): WaveletMatrixUint8 {
    const { shape, payloads } = decodeSnapshot(
      snapshot,
      SnapshotKind.WaveletMatrixUint8,
      1,
      3,
    );
  ...
  serialize(): Uint8Array {
    this.#assertAlive();
    return encodeSnapshot(SnapshotKind.WaveletMatrixUint8, [this.length], [
      new Uint8Array(
        memory.buffer,
        this.#bitsAllocation.pointer,
  ...
  accessMany(
    indices: Uint32Array,
    output: Uint8Array = new Uint8Array(indices.length),
  ): Uint8Array {
    this.#assertAlive();
  ...
  quantileMany(
    ...
    rights: Uint32Array,
    kths: Uint32Array,
    output: Uint8Array = new Uint8Array(lefts.length),
  ): Uint8Array {
    this.#assertAlive();
    if (
  ...
  #countLessThan(left: number, right: number, value: number): number {
    if (value === 0) return 0;
    if (value === UINT8_LIMIT) return right - left;
    return wasmCountLessThan(...this.#queryBase(), left, right, value);
  }

  #assertAlive(): void {
    if (this.#disposed) throw new Error("WaveletMatrixUint8 has been disposed");
  }
...
function validateValues(values: ArrayLike<number>): void {
  if (values instanceof Uint8Array) return;
  for (let index = 0; index < values.length; index++) validateUint8(values[index]!, "value");
}

function validateUint8(value: number, name: string): void {
  if (!Number.isSafeInteger(value) || value < 0 || value >= UINT8_LIMIT) {
...
function validateBound(value: number, name: string): void {
  if (!Number.isSafeInteger(value) || value < 0 || value > UINT8_LIMIT) {
    throw new RangeError(`${name} must be in [0, 256]`);
  }
}
