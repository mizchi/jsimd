...
function nodeKey(expression: F32Expression): string {
  switch (expression.kind) {
    ...
    default: {
      const left = nodeKey(expression.left);
      const right = nodeKey(expression.right);
      // Preserve operand order. This keeps NaN payload and signed-zero behavior
      // tied to the submitted floating-point plan.
