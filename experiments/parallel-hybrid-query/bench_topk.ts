import { ParallelHybridVectorIndex, type TopKSelector } from "./parallel_index.ts";
...
const WARMUPS = Number(Deno.env.get("JSIMD_HYBRID_WARMUPS") ?? 10);
const SAMPLES = Number(Deno.env.get("JSIMD_HYBRID_SAMPLES") ?? 31);
const K_VALUES = [1, 10, 100] as const;
...
for (const selectivity of [0.01, 0.1, 1]) {
  ...
  for (const k of K_VALUES) {
    let reference = "";
    for (const selector of ["javascript", "wasm"] satisfies TopKSelector[]) {
      const durations = await measure(async () => {
        const result = await index.searchBetween(query, 0, maximum, {
          k,
          plan: "filter-first",
          selector,
        });
        const signature = `${result.ids.join(",")}|${result.distances.join(",")}`;
        if (reference === "") reference = signature;
        else if (signature !== reference) throw new Error(`${selector} disagreed with reference`);
      });
      measurements.push({
        selectivity,
        k,
