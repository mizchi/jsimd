...
import { cellFromFixedPoint } from "../life_game.ts";
...
import { PixelWorkerControl } from "../pixel_worker_control.ts";
...
self.onmessage = (event: MessageEvent<PixelWorkerInitMessage>) => {
  ...
  void initialize(input, control, event.data.canvas, event.data.occupancy, event.data.region)
    .catch((error: unknown) => {
...
async function initialize(
  ...
  const simulation = ActivePixelSimulation.create(cells, control.width, control.height);
  ...
  const activateStroke = (fromX: number, fromY: number, toX: number, toY: number): void => {
  ...
  const runCycle = (): { readonly wakeSequence: number; readonly timeout?: number } => {
    ...
    for (let record = 0; record < discreteCount; record++) {
      ...
      if (kind === ATOMIC_INPUT_KIND.pointerDown) {
        ...
        dragMaterial = button === 2 ? MATERIAL.empty : asMaterial(control.brushMaterial);
    ...
    if (control.running && now >= nextStepAt) {
      ...
      tick++;
      changed = true;
      // Include compute/present time in the requested cadence instead of adding it on top.
    ...
    return {
      ...
      timeout: control.running ? Math.max(0, nextStepAt - performance.now()) : undefined,
    };
  };
...
function asMaterial(value: number): PixelMaterial {
  if (value === MATERIAL.empty || value === MATERIAL.wall || value === MATERIAL.sand) return value;
