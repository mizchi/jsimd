...
/** An immutable sixteen-level binary wavelet matrix over the complete unsigned 16-bit domain. */
export class WaveletMatrixUint16 {
  readonly length: number;
  ...
  static from(values: ArrayLike<number>): WaveletMatrixUint16 {
    return new WaveletMatrixUint16({ values });
  }

  static fromSnapshot(snapshot: Uint8Array): WaveletMatrixUint16 {
    const { shape, payloads } = decodeSnapshot(
      snapshot,
      SnapshotKind.WaveletMatrixUint16,
      1,
      3,
    );
    ...
    return new WaveletMatrixUint16({
      length,
      bits: payloads[0]!,
      ranks: payloads[1]!,
      zeros: payloads[2]!,
  ...
  get encodedBytes(): number {
    return LEVELS * (this.#paddedWords + this.#superblocks + 2) * 4;
  }

  serialize(): Uint8Array {
    this.#assertAlive();
    return encodeSnapshot(SnapshotKind.WaveletMatrixUint16, [this.length], [
      new Uint8Array(
        memory.buffer,
        this.#bitsAllocation.pointer,
  ...
  accessMany(
    indices: Uint32Array,
    output: Uint16Array = new Uint16Array(indices.length),
  ): Uint16Array {
    this.#assertAlive();
    ...
    try {
      ...
      const resident = new Uint32Array(memory.buffer, outputPointer, output.length);
      for (let index = 0; index < output.length; index++) output[index] = resident[index]!;
      return output;
    } finally {
      allocator.release(scratch);
    }
  ...
  quantileMany(
    ...
    rights: Uint32Array,
    kths: Uint32Array,
    output: Uint16Array = new Uint16Array(lefts.length),
  ): Uint16Array {
    this.#assertAlive();
    if (
  ...
  #countLessThan(left: number, right: number, value: number): number {
    if (value === 0) return 0;
    if (value === UINT16_LIMIT) return right - left;
    return wasmCountLessThan(...this.#queryBase(), left, right, value);
  }

  #assertAlive(): void {
    if (this.#disposed) throw new Error("WaveletMatrixUint16 has been disposed");
  }
...
function validateValues(values: ArrayLike<number>): void {
  if (values instanceof Uint16Array) return;
  for (let index = 0; index < values.length; index++) validateUint16(values[index]!, "value");
}

function validateUint16(value: number, name: string): void {
  if (!Number.isSafeInteger(value) || value < 0 || value >= UINT16_LIMIT) {
...
function validateBound(value: number, name: string): void {
  if (!Number.isSafeInteger(value) || value < 0 || value > UINT16_LIMIT) {
    throw new RangeError(`${name} must be in [0, 65536]`);
  }
}
